<?php
class Ticketsystemclass extends CI_Model
{
	//DebitsAgainst_Validation
	function CreateTicket_Validation()
	{
		...
		$this->form_validation->set_rules('issueticket', 'DebitsAgainst_Code', 'required');
	...
	function CreateTicket_Save()
	{
		$qry = "cosmo_support.dbo.Create_issueTicket '" . $_POST['module_id'] . "'
			,'" . $_POST['office_dbname'] . "'
			,'" . $_POST['User_Id'] . "'
			,'" . $_POST['issueticket'] . "'
			,'" . $_POST['User_Id'] . "'
		...
		if ($query) {
			$this->session->set_flashdata('msgS', 'Success!!!');
				...
				$this->session->set_flashdata('msgD', "Not Saved");
			}
		echo "<script> history.go(-1);</script>";
	}
	//Bank
...
";
	...
	function Truck_Save()
	{
		$qry = "" . $_SESSION['currentdatabasename'] . ".dbo.Exec_Truck '" . $_POST['name'] . "'
	,'" . $_POST['code'] . "'
	,'" . $_POST['description'] . "'
