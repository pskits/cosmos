...
<div class="content-wrapper">
    <section class="content-header">
        <h1>Accounts Collection</h1>
    </section>
    <section class="content">
        <div class="box box-form box-solid">
            <div class="box-header with-border">
                <h3 class="box-title">Accounts Collection Details</h3>
                <div class="box-tools pull-right">
                    <a href="<?php echo site_url('Accounts/Accounts_Collection_View'); ?>" class="btn btn-flat"><i class="fa fa-search"></i> View</a>
                    <a href="<?php echo site_url('Accounts/Accounts_Collection'); ?>" class="btn btn-flat "><i class="fa fa-refresh"></i> Refresh</a>
                </div>
            </div>
            <?php echo form_open_multipart(site_url('Accounts/Accounts_Collection_') . $But . '/', 'role="form"'); ?>
            ...
            <input type="hidden" name="collection_status_id" id="collection_status_id" value="2" />
            <div class="box-body">
                <div style="color:red;">
                ...
                <div class="row">
                    <div class="col-md-6">
						...
						<div class="form-group">
							...
							<input type="text" autocomplete="off" minlength="3" class="form-control input-md" id="Bank_ref" required name="Bank_ref" placeholder="Enter Bank Reference" value="<?php echo @set_value('Bank_ref') . @$Bank_ref; ?>">
					...
					<div class="col-sm-6 ">
						<div class="form-group">
                            <label>Bank</label>
                            <select id="Bank_Id" required class="form-control select2" name="Bank_Id" style="">
                                <?php
                                $data = $this->GM->Bank();
...
<script>
    ...
    function cheque() {
		...
		if (id == '2') {
            $(".cash_segment").show();
            $(".cash_segment").find("input").prop("required", true);
        } else {
            $(".cash_segment").hide();
            $(".cash_segment").find("input").removeAttr("required");
        }
    }
    function Addrow(data, payableamount = 0.00) {
        ...
        for (i = 0; i < data.length; i++) {
            ...
            cell3.innerHTML = `<input type="number"  class="form-control input-md InvoicePayAmount" id="InvoicePayAmount" Value="` + payableamount + `" step="0.01" min="0.00"  onkeypress="if ( isNaN( String.fromCharCode(event.keyCode) )) return false;" required name="InvoicePayAmount[` + rows + `]" >`;
        }
    }
</script>
<script>
    function Dealerinvoicelist(e) {
        ...
        if (Id) {
            $(function() {
                $.ajax({
                    type: 'GET',
                    url: '<?php echo site_url('API/AccountDealerinvoicelist'); ?> ',
                    data: {
                        Dealer_Id: Id
                    },
