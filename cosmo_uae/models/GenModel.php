...
class GenModel extends CI_Model
{
	...
	//Get_AllowtoprocessType
	function AllowtoprocessType($AllowtoprocessTypeId = "0")
	{
		$res = $this->db->query('EXECUTE cosmo.dbo.Get_AllowtoprocessType ' . $AllowtoprocessTypeId);
		return $res->result();
	}
...
//Get_GoodsInvoiced
	...
	//Goods
	function Goods($Goods_Id = "0", $Category_Id = "0", $Product_Id = "0", $Serial_No = "", $GoodsStatus_Id = "0", $status_Id = "1", $warehouse_id = "0")
	{
	...
	//Collection
	function Collection($status = "1", $dealer = "0", $collectedUser_Id = "0", $portal_Id = "0", $process_Id = "0", $amountmode_Id = "0", $CollectionStatus_Id = "0", $Id = "0", $from_date = '', $to_date = '', $accountstatus='0')
	{
		$res = $this->db->query('EXECUTE '.$_SESSION['currentdatabasename'].'.dbo.Get_Collection ' . $status . ',' . $dealer . ',' . $collectedUser_Id  . ',' . $portal_Id . ',' . $process_Id . ',' . $amountmode_Id . ',' . $CollectionStatus_Id . ',' . $Id . ",'" . $from_date . "','" . $to_date . "','" . $accountstatus . "'");
		// echo $this->db->last_query();exit;
		return $res->result();
	}
	//Get_CreditNote
	...
	//Orderlist
	function Orderlist($status_id = 1, $Warehouseid = "0", $Dealer_Id = "0", $salesexecutiveuser_Id = "0", $priority_Id = "0", $Order_status_Id = "0", $Id = "0", $from_date = '', $to_date = '', $Accountstatus='0')
	{
		$res = $this->db->query("EXECUTE ".$_SESSION['currentdatabasename'].".dbo.Get_TransOrder " . $status_id . ',' . $Warehouseid . ',' . $Dealer_Id . ',' . $salesexecutiveuser_Id . ',' . $priority_Id . ',' . $Order_status_Id . ',' . $Id . ",'" . $from_date . "','" . $to_date . "', '". $Accountstatus ."'");
		// echo $this->db->last_query();exit;
		return $res->result();
	}
	...
	/*************************** Ac direct deal *******************************/
	//Accounts Collection
	function AccountsCollection($status = "1", $dealer = "0", $collectedUser_Id = "0", $portal_Id = "0", $process_Id = "0", $amountmode_Id = "0", $CollectionStatus_Id = "0", $Id = "0", $from_date = '', $to_date = '', $accountstatus='0')
	{
		$res = $this->db->query('EXECUTE '.$_SESSION['currentdatabasename'].'.dbo.Get_Collection ' . $status . ',' . $dealer . ',' . $collectedUser_Id  . ',' . $portal_Id . ',' . $process_Id . ',' . $amountmode_Id . ',' . $CollectionStatus_Id . ',' . $Id . ",'" . $from_date . "','" . $to_date . "','" . $accountstatus . "'");
		// echo $this->db->last_query();exit;
		return $res->result();
	}	
	//AccountsInvoiceAmountInfo
	function AccountInvoiceDue($status_id = 1, $Dealer_ID = "0", $Invoice_Id = "0", $salesexecutiveid = "0")
	{
		$res = $this->db->query("EXECUTE ".$_SESSION['currentdatabasename'].".dbo.Get_AccountInvoiceDue " . $status_id  . ',' . $Dealer_ID . ',' . $Invoice_Id . ',' . $salesexecutiveid);
		// echo $this->db->last_query();exit;
		return $res->result();
	}
	
	//AccountsDealerDueAmountInfo
	function AccountsLedgerDetails()
	{
	 	$res = $this->db->query("Select * FROM ".$_SESSION['currentdatabasename'].".dbo.vw_OfficelegerableList where Ledgername IS NOT NULL AND Ledgername<>''");
		//echo $this->db->last_query();//cexit;
		return $res->result();
	}
}
?>