...
?>
          ...
          <div class="col-12">
            <div class="callout callout-info row">
              ...
              <div class="col-md-8">
                ...
                ?>
                  <a onclick="askDecline()" style="background-color:#dc3545;margin-right:1px;" class="btn btn-flat pull-right"><i class="fa fa-refresh"></i> Reject</a>
                  <a onclick="askapprovalandinvoice()" style="background-color:#0ea3a9;margin-right:1px;" class="btn btn-flat pull-right"><i class="fa fa-refresh"></i> Approve </a>
				  <a <?php echo "href='" . site_url('') . "/" . "sales/salesorder/?Key=" . base64_encode($Row->Order_Id) . "'"; ?> style="background-color:#0ea3a9;margin-right:1px;" class="btn btn-flat pull-right"><i class="fa fa-pencil"></i>Edit
                </a> 
			  ...
			  <?php
                ...
                if ($Row->Order_status_Id == '3') {
					                ?>
									  <a onclick="OrderCashRate_form()" style="background-color:#0ea3a9;margin-right:1px;" class="btn btn-flat pull-right"><i class="fa fa-refresh"></i> Apply CashRate </a>
                ...
                <a href="<?php echo site_url('Sales/Orderlist'); ?>" style="background-color:#0ea3a9;margin-right:1px;" class="btn btn-flat pull-right"><i class="fa fa-arrow-circle-left"></i> </a>
  ...
  <iframe style="width:0px;height:0px;" src="<?php echo site_url('Sales/SalesOrderPrint/?Key=' . $_GET['Key']); ?>" name="frame"></iframe>
    <form id="OrderCashRate_form" method="POST" action="<?php echo site_url('Sales/Orders_CashRate'); ?>">
	...
	<script>
	   ...
	   function OrderCashRate_form() {
          var r = confirm("This action will Change Product Price as Cash Rate !");
          if (r == true) {
            document.getElementById("OrderCashRate_form").submit();
          }
        }
		</script>
  <?php
  ...
  ?>
      <script>
        ...
        function askapprovalandinvoice() {
          var r = confirm("This action will approve this order !");
          if (r == true) {
            document.getElementById("approveandinvoice_form").submit();
          }
        }
      </script>
      <form id="approveandinvoice_form" method="POST" action="<?php echo site_url('Sales/Order_Accept'); ?>">
        <input type="hidden" name="User_Id" value="<?php echo $this->session->userdata['cosmolog']['UId'] ?>" />
    ...
    <script>
      function askDecline() {
        var r = confirm("Order Will be Declined!");
        if (r == true) {
          document.getElementById("Decline_form").submit();
        }
      }
    </script>
    <form id="Decline_form" method="POST" action="<?php echo site_url('Sales/Order_Decline'); ?>">
      <input type="hidden" name="User_Id" value="<?php echo $this->session->userdata['cosmolog']['UId'] ?>" />
      <input type="hidden" name="Order_Status_Id" id="Order_Status_Id" value="4" />
      <input type="hidden" name="Order_Id" id="Order_Id" value="<?php echo $Row->Order_Id ?>" />
    </form>
<?php
  }
