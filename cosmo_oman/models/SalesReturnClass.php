...
class SalesReturnClass extends CI_Model
{
    ...
    {
        $qry = "" . $_SESSION['currentdatabasename'] . ".dbo.Exec_TransSalesReturnRequestProductStatus '" . $_POST['SerialNo'] . "','" . $_POST['SalesReturnRequest_Id'] . "', '" . $_POST['Status'] . "','" . $_POST['User_Id'] . "'	";
        $query = $this->db->query($qry);
        if ($query) {
            $this->session->set_flashdata('msgS', 'Success!!!');
        } else {
    ...
    function SalesReturn_Save()
    {
		...
		if ($lastid) {
			...
			Exec " . $_SESSION['currentdatabasename'] . ".dbo.Exec_TransSalesReturnReplaceable 
		 '" . $lastid . "','" . $_POST['User_Id'] . "'	;";
		 $prdquery = $this->db->query($prdqry);
