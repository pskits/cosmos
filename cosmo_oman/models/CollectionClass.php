...
class CollectionClass extends CI_Model
{
	...
	function Collection_Save()
	{
		...
		,'1'
		,'" . $_POST['Abut'] . "'
		,'" . date('m-d-Y',strtotime($_POST['CollectionDate'])) . "'		
		";
		//print_r($qry);  ,'" . $_POST['CollectionDate'] . "'	
	...
	{
		 $qry = "Select tdc.Collection_Id from ".$_SESSION['currentdatabasename'].".dbo.Trans_DepositCollection tdc 
			where tdc.Deposit_Id ='" . $_POST['Deposit_Id'] . "'
			and tdc.Collection_Id not in (
SELECT tv.Against_Id FROM ".$_SESSION['currentdatabasename'].".dbo.Trans_Voucher tv WHERE tv.VoucherSource_Id=1)";
		$query = $this->db->query($qry);
		$res = $query->result();
		$rowqry = "Exec ".$_SESSION['currentdatabasename'].".dbo.Exec_AccountsDeposit " . $_POST['Deposit_Id'].";";
		foreach ($res as $row) {
		$rowqry.= "Exec ".$_SESSION['currentdatabasename'].".dbo.Exec_AccountsCollection " . $row->Collection_Id.";"; 
		}			
...
SELECT tv.Against_Id FROM ".$_SESSION['currentdatabasename'].".dbo.Trans_Voucher tv WHERE tv.VoucherSource_Id=1)";
		...
		$rowqry = "";
		foreach ($res as $row) {
		$rowqry.= "Exec ".$_SESSION['currentdatabasename'].".dbo.Exec_AccountsCollectionBank " . $row->Collection_Id; 
		}	
